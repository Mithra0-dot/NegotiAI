...
import { OpponentPresetPicker } from "../components/OpponentPresetPicker";
import type { ChatMessage, ChatTurn, SessionScore, StrategyVariant } from "../types/chat";

export function ChatPage() {
  const { scenarioId } = useParams<{ scenarioId: string }>();
  const scenario = scenarios.find((s) => s.id === scenarioId);

  // Gate: null until the user picks an opponent preset, per scenario
  // selection — see the approved plan's "step after scenario selection,
  // not baked into the picker grid" UX decision.
  const [variant, setVariant] = useState<StrategyVariant | null>(null);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!text || !scenarioId || !variant || isSending || sessionScore) return;
    ...
    try {
      const { reply, phase, tactic, detected_signals, session_score } =
        await sendChatMessage(scenarioId, text, turnNumber, history, variant);
      setMessages((prev) => {
        const next = [...prev];
  ...
  return (
    <div className="mx-auto flex h-screen max-w-2xl flex-col px-6 py-8">
        ...
        <div className="mt-2 flex items-center gap-3">
          ...
          {variant && (
            <span className="inline-flex items-center rounded-full bg-violet-500/15 px-2.5 py-0.5 text-xs font-medium text-violet-300 ring-1 ring-inset ring-violet-500/30">
              {opponentPresets.find((p) => p.variant === variant)?.label}
            </span>
          )}
        </div>
        {scenario && (
      ...
      {!variant ? (
        <OpponentPresetPicker onSelect={setVariant} />
      ) : (
        <>
